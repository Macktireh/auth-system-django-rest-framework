...
def send_email_activation_account(request, user):
    current_site = get_current_site(request)
    email_subject_activate = f"Confirmer votre adresse email de votre compte Mackdin"
    email_body_activate = render_to_string('accounts/actiavte.html', {
            'user': user,
            'domain': current_site,
...
def send_email_activation_account_success(request, user):
    current_site = get_current_site(request)
    email_subject_activate_success = f"{current_site} - Votre compte a été créé et activé avec succès !"
    email_body_activate_success = render_to_string('accounts/actiavte_success.html', {'domain': current_site,})
    email = EmailMessage(
